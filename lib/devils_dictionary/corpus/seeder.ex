defmodule DevilsDictionary.Corpus.Seeder do
  @moduledoc """
  Seeds a committed corpus manifest into the existing encyclopedia structures.

  Two kinds of content (#174): **works** — artworks, poems, books, the
  corpora under `priv/artworks/manifests/` — and **quotations**, the
  public-domain Wikiquote corpus under `priv/quotes/manifests/`. It was
  `DevilsDictionary.Artworks.Corpus.Seeder` until the second kind arrived;
  that name delegates here for one release.
  ...
  A `wikiquote-pd` row is a `content/quotation` entry in the shape the live
  Wikiquote provider writes, identified by its fingerprint and credited
  `authored_by` by QID, so a corpus line and a live line with one wording are
  one subject. Its source record, its evidence and its badge are
  `Quotations.Corpus.seed_row/3`'s. The authors are prepared from the
  manifest's own Wikidata facts, so seeding still makes no request.
  """
  ...
  def run(manifest, opts \\ []) when is_map(manifest) do
    ...
    context = if opts[:dry_run], do: nil, else: context(kind, manifest)
        ...
        case entry(kind, version, row) do
          {:ok, entry} ->
            if opts[:dry_run] do
              ...
              # Only a work's display facts are the manifest's to rewrite; a
              # quotation's text is a revision and is never refreshed.
              owned? =
                opts[:refresh] && entry.object_kind == :entity && corpus_owned?(entry, version)

              resolution = resolve(kind, entry, row, context)
  ...
  # What a kind's rows share. Works share nothing: the entry is the whole write.
  defp context("wikiquote-pd", manifest), do: Quotations.Corpus.seed_context(manifest)
  defp context(_kind, _manifest), do: nil

  defp resolve("wikiquote-pd", entry, row, context),
    do: Quotations.Corpus.seed_row(entry, row, context)

  defp resolve(_kind, entry, _row, _context), do: SourceIdentity.resolve(entry)
  ...
  # A quotation (#174): the live provider's entry shape (`Wikiquote.identity_record/1`)
  # with the fingerprint as the stable identifier — see `Quotations.Corpus`
  # for why not the positional `wikiquote_item` — and the credit at
  # `:verified`, because the build kept the line only when a text its author
  # wrote contains it.
  @doc false
  def entry("wikiquote-pd", version, row) do
    with {:ok, text} <- required(row["text"]),
         {:ok, fingerprint} <- required(row["fingerprint"]),
         {:ok, author} <- required(row["author_qid"]),
         true <- Fingerprint.fingerprint(text) == fingerprint || {:error, :fingerprint_mismatch},
         true <- valid_qid?(author) || {:error, :invalid_author_qid} do
      Entry.new(%{
        source_slug: Quotations.Corpus.slug(),
        object_kind: :content,
        content_kind: :quotation,
        stable_identifier: %{namespace: "quotation_fingerprint", external_id: fingerprint},
        ...
        metadata: %{
          "author_display_name" => row["author_label"],
          "locator" => row["locator"],
          "corpus" => version
        },
        relationships: [
          %{
            ...
            certainty: :verified,
            target_identifiers: [
              %{namespace: "wikidata", external_id: author, metadata: %{"field" => "P50"}}
            ]
          }
        ],
        eligibility: :eligible,
