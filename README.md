...
| **#102 Phase 1** the provider contract hardened ([PR #104](https://github.com/razrfly/dictionary/pull/104)): registry from config, `Req.request` with GET, the content-type table (`Discovery.ContentTypes`) replacing every `[:film, :gif]` branch, `shelf_detail/0`, `retryable_status?/1` + `min_retry_interval_ms`, a fake GET/offset/text provider on the same pipeline; 978 tests. Audited A− | ✅ 2026-09-17 |
| **#102 Phase 2a** the Met through the pipeline ([PR #107](https://github.com/razrfly/dictionary/pull/107)): `Providers.Met` matched by **tag-QID identity** (text search only generates candidates), the ≤2-step P31/P279 broader walk, `covers?/1`, `mix dd.links.reinstate` (14,116 `refers_to` links back to active; 15,010 lexemes carry a QID); ≤61 live requests, no image bytes; 1,016 tests. Audited A | ✅ 2026-09-17 |
| **#102 Phase 2b** the durable art corpus ([PR #108](https://github.com/razrfly/dictionary/pull/108)): committed, checksummed manifests `met-highlights-v1` (1,644, PD gated on the hydrated object) and `wikidata-famous-v1` (1,575, threshold 10 measured by binding sitelink counts with `VALUES`), `Artworks.Corpus.Seeder` idempotent on the #86 structures, a QID path in `Artworks.suggestions/1`; **the Met throttles on volume** (44% refused at 1 req/s, none at 3 s); 3,212 artworks, 0 migrations, 1,052 tests. Audited A− | ✅ 2026-09-17 |
