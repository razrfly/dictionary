...
## Current delivery — 17 September 2026

The discovery layer landed on 12–13 September in four PRs: automatic CineGraph film discovery on definition pages ([#88](https://github.com/razrfly/dictionary/issues/88) / [PR #89](https://github.com/razrfly/dictionary/pull/89)), the shared cached-discovery lifecycle with GIPHY as a browser-only shelf ([#92](https://github.com/razrfly/dictionary/issues/92) / [PR #94](https://github.com/razrfly/dictionary/pull/94)), durable source identity for discovered films ([#93](https://github.com/razrfly/dictionary/issues/93) / [PR #95](https://github.com/razrfly/dictionary/pull/95)) and the Wikidata-linked Artsy artwork catalog ([#86](https://github.com/razrfly/dictionary/issues/86) / [PR #98](https://github.com/razrfly/dictionary/pull/98)). The 17 September triage ([#102](https://github.com/razrfly/dictionary/issues/102)) grades it **B**: only CineGraph runs through the discovery pipeline (`Discovery.Providers.server_providers/0` returns `["cinegraph"]`), Artsy is a seeded catalog on its own client, GIPHY has no recorded live verification, and three shared-code assumptions — POST-only transport, cursor-only pagination, a `[:film, :gif]` allowlist — stand between the Met and a cheap second provider. #102 sequences what comes next: harden the provider contract, then the Met ([#99](https://github.com/razrfly/dictionary/issues/99)), then one text provider ([#100](https://github.com/razrfly/dictionary/issues/100)), then the category-grouped UI and [#96](https://github.com/razrfly/dictionary/issues/96), then the assessor ([#101](https://github.com/razrfly/dictionary/issues/101)) and personas ([#97](https://github.com/razrfly/dictionary/issues/97)). The development database is `devils_dictionary_v2`; `devils_dictionary_dev` is the pre-#74 baseline and has no discovery tables.
...
Where it stands (2026-09-07, MVP-0 closed): the lexicon and its relations, and the encyclopedia side with them, are built and on the word page; every card says where it came from. The culture is planned — the `users` + `examples` + `votes` migration was written, applied, diffed and rolled back in S5 to prove it touches none of the thirteen tables — and `?demo=1` on any word page draws it, and the sources nobody has absorbed yet, as clearly-labelled samples so #66 and #67 have a layout to argue with.

---

## Status

**MVP-0, the walking skeleton, is done** (2026-09-07, tag `mvp-0`). Spec: [#69](https://github.com/razrfly/dictionary/issues/69). Build tracker (reset procedure, sessions S0–S5, directory structure): [#70](https://github.com/razrfly/dictionary/issues/70). UI tracker (U0–U3): [#71](https://github.com/razrfly/dictionary/issues/71). Six sources (Open English WordNet, Wiktionary via Kaikki, Wikidata, Wikipedia, Bierce and — as S5's extensibility proof — Johnson 1755), two scopes (Animals, and Emotions as the toy), thirteen tables, six plain pages, and a scorecard that `mix dd.score` computes.

MVP-0 was done when every scorecard row passed, and it does: **animals 36 / 36 graded rows with parity, 1 reported, 0 pending; emotions 35 / 35, 2 reported, 0 pending**, on 487 tests.
...
| U1b the thing side (`30a1e8c`): the concept card, the chain (taxon first, subclass as fallback, instance only at the first step, one parent per step), kinds and examples that have a word, *may refer to*, the disagreement plaque; three `Encyclopedia` reads; the U1a deductions cleared; **S5c landed with it** (scoped concept seed: emotions 72,108 → 414 QIDs; per-scope bars; L3 *n/a* without a root; `?scope=` on every page) — animals **34 / 34**, emotions **32 / 32**, 422 tests. Audited **A**: every number re-measured, driven in Chrome on *cat*, *joy* and *seal*. | ✅ 2026-09-07 |
...
| U2 home, search, provenance and the sparse states: `/` is a `HomeLive` with trigram search over the whole index, *Surprise me* and the seed words (`PageController` gone); the **ⓘ provenance drawer** on every card and on the thing panel, addressed by `?provenance=card:<id>[:n]` and resolved against the page, showing the record's external id, url, hash, timestamps, license, the concept links with method and confidence, and the trimmed raw; the four sparse states finished — *did you mean* on a miss, a bare row that reads like a promise, *redirected from*, and **in Animals** / **not in Animals or Emotions · WordNet only**. Row **U3 is measured**, not asserted (`Health.Pages.cards_provenance/0`), and U6's population grew by the panel's two ↗. 458 tests; **animals 35 / 35, emotions 34 / 34**. | ✅ 2026-09-07 |
| U2 audited (grade A): 458 tests, animals **35 / 35 with parity**, emotions 33 / 33; U3 *43 / 43 cards, 156 / 156 citations*, U6 *53 / 53* with the thing panel's links, X2 p95 66 ms; home, search, *Surprise me*, the drawer on every card kind and the four sparse states driven in Chrome at 1280 px and 375 px. **Only U4 remains; U3 closes MVP-0.** | ✅ 2026-09-07 |
| U3 fake-data mode, the mobile pass and the close of MVP-0: **`?demo=1`** (dev and test only) renders the layers nobody has absorbed — Webster 1913, EB1911, Urban Dictionary and the #67 evidence wall — as dashed SAMPLE cards through the *real* components, merged in `WordLive.load/3` so `WordPage.build/2` and therefore the scorecard can never see one; **row U4** graded as a dated attestation with twelve 375 px screenshots in [`docs/mobile/`](docs/mobile/README.md), falsifiable the way E3 is; a **chip-click test for all ten** of #71 §7's relation groups plus a sense-scoped chip and a chain step. The two acceptance walks driven on the real index: ten hops from *oyster* and ten from *joy*, no dead end, every card with ↗ and ⓘ. **animals 36 / 36 with parity, emotions 35 / 35, 0 pending; 487 tests** (from 458). | ✅ 2026-09-07 |
| **#74 P0** the encyclopedia model decided and evidenced (`10097c4`): [ADR 0001](docs/adr/0001-encyclopedia-model.md), the [port checklist](docs/rebuild/port-checklist.md), the [score-row mapping](docs/rebuild/score-rows.md), `priv/sources/MANIFEST.json` at 5/5 verified digests, `mix dd.export.replay` (101 MB, including the Wikipedia and Wikidata records that exist **only** in a database), and the [Gate 0 spike](docs/spikes/2026-09-gate0/README.md) — integrity rejections, the current-revision shootout that chose a partial `is_current` index over a pointer (0.256 ms vs 2.576 ms p95; 41 vs 7,957 buffers), and the Wiktionary `bank` sense-identity reproduction | ✅ 2026-09-08 |
| **#74 P1** the port onto the new model (`6a6ae8f`) and the [completion audit](https://github.com/razrfly/dictionary/issues/74)'s seven correctness gaps closed: `revise/2` carries every context column and flips only `is_current` (currentness and lifecycle are different questions); public reads exclude rejected claims **before** counting and paging; keyset pagination; review-context items are tied to their endpoint by a trigger; `merge/3` moves names and identifiers and `split/3` opens a case per attachment rather than guessing; `Registry.Object` declares its subtypes. Plus `raw_sql_test.exs`, which prepares every raw statement in the codebase against the test database — `mix compile` cannot see a stale table name, and it found one on its first run | ✅ 2026-09-08 |
| **#74 P2/P4** the write path and the durability proofs (`e60ab6e`): content-matched sense identity, output ownership by `last_seen_run_id`, the rewritten `Resolver` and `Linker` (a rerun may not resurrect a rejected link, nor lower a confidence corroboration already raised), and six new scorecard rows — D1 refresh reconciles, D2 meaning identity is durable, D3 editorial decisions survive, D4 review context is pinned, D5 integrity holds on bulk writes, D6 inputs are pinned | ✅ 2026-09-08 |
| **#74 P3** one connected flow (`0dbb752`): `/words/:id/:slug` and `/entities/:id/:slug` canonical, `/define/:slug` a resolver that *offers the choice* where a slug names more than one lemma, `/connections/:id` readable from either endpoint, and `/connect` the authenticated composer whose relation list is read off `predicate_endpoint_rules`. The acceptance walk is real clicks: nepotism → Bierce's definition → Bierce → his works → the edition → back | ✅ 2026-09-09 |
| **#79 W1** the corpus spike and the budgets ([evidence](docs/rebuild/w1-2026-09-10/README.md)): `mix dd.absorb wiktionary --sample-every N` samples by hash of the headword, not by position, because the dump is in page order and `--limit` takes a biased head. Two near-disjoint samples (N=30, N=31, **94,034 records**) put the full pass at **~15 min, ~3 GB, projected DB ~9.5 GB**, and coverage at **12.7 % → low-to-mid 90s**; the census corrects #79's guess — **1,487,639** English records, not ~1.3 M. The spike also found the defect the three test scopes had hidden: `origin_key` is `varchar(255)` and Wiktionary's `coordinate_terms` names a whole series in one target (`A-shaped - … - Z-shaped`, 315 bytes), aborting the entire materialize transaction — 1 key in 94,034 records, fatal every time. All four builders now share a bounded key (readable head + digest tail): deterministic, inspectable, **no migration**. 725 tests | ✅ 2026-09-10 |
| **#79 N1** the prototype surface stripped (#77 §1, §2): the navigation was `Animals · Health · Imports` — one test population and two consoles standing in for the product's structure — and it is now a logo and a theme toggle. **24 sites.** The three consoles moved to `/ops/{scopes/:slug,health,imports}`, with the old paths kept as **302s** (not 301s: `/ops` is a first guess, and a browser caches a permanent redirect near-forever). **15 production `\\ "animals"` defaults removed** and the argument made required — `Health`'s moduledoc now names which rows are whole-corpus and which are one population's — and all four `--scope` flags refuse a missing selection, `dd.rebuild` **before** it builds a plan, because its stages 8–9 go to the API for hours. No selection is a state the consoles render, not a value they invent. Public copy lost the internal names: the homepage reads *1,541,668 words indexed · 282,098 with at least one definition · 6 sources so far*, and `/define/ambrose-bierce` reads *One source so far · Open English WordNet 2025* instead of *not in Animals or Culture or Emotions*. Found while there: `mix dd.health --scope emotions` computed its footnote off a second, Animals-scoped ledger; Animals' twelve Wiktionary category exclusions were compiled into a command that accepts any scope, and are now `priv/scopes/animals.json`'s; and the kit's buttons had no disabled styling, so the absorb button that refuses an unscoped API import still offered the hand cursor. **Animals stays as ordinary content** — a test deletes the scope row and walks the app. 752 tests (from 725); animals 43/44, culture 41/42, emotions 42/43, X3 failing identically on `main`; U4 re-attested at 375 px, all twelve **0** | ✅ 2026-09-10 |
| **#88** automatic CineGraph discovery on definition pages ([PR #89](https://github.com/razrfly/dictionary/pull/89)): `discovery_mappings` / `discovery_runs` / `discovery_results`, the `Discovery.Provider` behaviour, a DB-coordinated rolling request budget, positive and negative caches with a freshness policy, the run lease and cleanup worker, `mix dd.discovery` and `mix dd.discovery.check`; **live-verified** against `cinegraph.org` ([handoff](docs/discovery/issue-88-handoff-2026-09-12.md)); audited B, repaired, re-audited **A+** ([re-audit](docs/audits/2026-09-12-issue-88-reaudit.md)); 875 tests | ✅ 2026-09-12 |
| **#92** unified cached discovery ([PR #94](https://github.com/razrfly/dictionary/pull/94)): per-source freshness overrides and shared quotas for the pipeline; GIPHY restored as a **browser-only, transient** shelf (`assets/js/giphy_shelf.mjs`) because GIPHY has not granted caching approval — no live verification recorded, and its `node:test` file is not wired to anything | ✅ 2026-09-12 |
| **#93** durable source identity for discovered films ([PR #95](https://github.com/razrfly/dictionary/pull/95)): the CineGraph / TMDb / IMDb / Wikidata crosswalk into `external_identifiers`, local film entries with backlinks, legacy-result backfill ([contract](docs/integrations/source-identity.md)); the identity adapter is duck-typed, so a new provider gets it without wiring | ✅ 2026-09-12 |
| **#86** the Wikidata-linked Artsy artwork catalog ([PR #98](https://github.com/razrfly/dictionary/pull/98)): `Artsy.Client` + `RequestCoordinator`, the `Artworks` context and seeder, `/artworks`, `mix dd.artworks.seed` / `mix dd.artsy.withdraw`, four manifests and the meaning → gene mappings; **0 migrations**; ≤194 live requests across the feasibility probe, the pilot and two audit-repair rounds (C+ → B → repaired; [repair](docs/audits/2026-09-12-issue86-repair.md), [re-audit repair](docs/audits/2026-09-13-issue86-reaudit-repair.md)); 9 artworks seeded; the definition page reads the local catalog only, so Artsy's retirement is survivable. Its provider module has no `retrieve/4` — the catalog runs outside the discovery pipeline | ✅ 2026-09-13 |
| **Discovery triage** ([#102](https://github.com/razrfly/dictionary/issues/102)): #73, #74, #75, #79, #86 and #93 closed with evidence comments; the layer graded **B**; the order fixed — provider-contract hardening, the Met, one text provider, the category-grouped UI, then the assessor and personas | ✅ 2026-09-17 |
...
- [x] Minimum word-hopping UI on the Oatmeal theme, theme boilerplate first, with a labelled fake-data mode (#71)
...
| 2026-09-06 | **E3 is an experiment, not an assertion.** `users` + `examples` + `votes` were generated with `mix ecto.gen.migration`, applied to the full development database, diffed against a `pg_dump -s` taken before them, and rolled back. The diff adds three tables, seven indexes and two check constraints and changes **no column, index or constraint of the thirteen**; four foreign keys point into `lexemes`, `senses`, `concepts` and `sources` without adding anything to them. The file lived at `docs/sketches/community_layer_migration.exs` until #74, where `mix ecto.migrate` could not see it. It is retired now, and the dated result above is preserved in `docs/sketches/README.md`: the community layer it sketched is **shipped schema** (`users`, `actors`, `assertion_reviews`, `assertion_votes`), so a rolled-back sketch of it is no longer evidence of anything. Scorecard row E3 became the extension exercise instead. `users` is included deliberately: there is no auth scaffolding, so a proof that skipped the awkward half would not be one. | #69 §4 §7 E3, #70 S5 |
...
| 2026-09-07 | U1b audited (A). Three rules from it stand: **a thing's chain prefers the taxonomy, falls back to subclass-of, follows instance-of only at the first step, and takes one parent per step** (the concept graph is a DAG; a plain walk fans out); **kinds and examples appear only when they have a word**, because a chip that cannot be clicked is furniture; **a scope's scorecard bars are its own data** (`scopes.rules["bars"]`), set as regression guards until argued from the sources. The concept seed defect from S5 is fixed and measured: an 809-word scope now seeds 414 QIDs, not 72,108. |
| 2026-09-07 | U2 audited (A). Two rules from it stand: **the provenance drawer is addressed by the card on the page, never by a row id** (a forged reference opens nothing; a row id in a URL is an enumerable handle), and **a page is not rebuilt to open a drawer** (a test counts the queries). U4, mobile, is graded as a dated attestation with screenshots, not pretended to be measured. U3's close-out is written into #71: both scopes scored with parity and pasted on #69, the map's status text updated, README *MVP-0 done*, tag `mvp-0`, issues closed. |
| 2026-09-07 | **The scorecard grades real rows only.** Fake-data mode merges its samples in `WordLive.load/3`, downstream of `WordPage.build/2` — and `Health.Pages` builds its pages by calling `build/2` directly, never through the LiveView, so no invented card can reach X1, U2, U3 or U6 however the mode is left switched on. The seam was chosen for that reason and a test asserts it. The corollary, found by looking at the page: the scope line's *N sources* is counted **before** the samples go in, because "8 sources" on a page where three are invented is a lie the banner cannot take back. | #71 U3 |
| 2026-09-07 | **A sample renders through the real component or it proves nothing.** The three sample cards are `%WordPage{}` cards in every field, so `Word.source_card/1` draws Webster 1913 exactly as it draws Bierce; what differs is a dashed border, a SAMPLE badge and a body that ends *(invented for layout)*. A parallel "demo card" component would have been easier and would have checked a layout that does not ship. The only sample with a component of its own is the evidence wall, because #67 has no component yet — which is the whole reason to draw it. | #71 U3, #66, #67 |
| 2026-09-07 | **`?demo=1` is threaded through the hop, not held in socket state.** A mode that evaporates on the first click is not a mode, and the drawer's own *close* link would have been the thing that turned it off. So `hop/3` and `info_path/4` carry it and every path-building component takes a `demo` attr — and `handle_params/3`'s rebuild guard gained it too, or the mode would not switch on a patch. The URL is the whole state of the page, as it is for the trail and the drawer. | #71 U3, §10 |
| 2026-09-07 | **U4 is a dated attestation with checked-in evidence, graded like E3.** No query can answer whether a page scrolls sideways on a phone; the instrument is a viewport and a pair of eyes. So `Score` reads twelve screenshots off `docs/mobile/`, names the date and the pages in `actual`, and **fails if one goes missing** — an attestation nobody can check is a claim, not a measurement. What was measured behind them: `scrollWidth - clientWidth == 0` at 375 × 812 with every `<details>` forced open, on all twelve. The things still wider than the viewport — the drawer's raw JSON, the health scorecard table — scroll inside their own `overflow-x-auto`, which is the rule and not an exception to it. Driven over CDP: `--window-size` sets the window, not the layout viewport, and page zoom in a real window makes `innerWidth` a lie. | #69 §7 U4, #71 U3 |
| 2026-09-07 | **MVP-0 is closed.** Every row of #69 §7 passes on both scopes with parity — animals 36 / 36, emotions 35 / 35, 0 pending — over 487 tests, and the two acceptance walks run on the real index: ten hops from *oyster* and ten from *joy*, no dead end, every card carrying ↗ and ⓘ. What the skeleton proved: thirteen tables through eight sessions and six sources of four access kinds; a scope is a JSON file; a new dead author costs one module and no migration; and the walk holds because every chip points at a resolved target and every word in the index has a page. What it did not: the culture layer is still a rolled-back sketch, the concept graph is one step deep for most non-taxa, and the design is #66's. | #69, #70, #71 |
| 2026-09-08 | **#74 P0: the schema is being rebuilt, and Gate 0 decided how.** The audit's findings all have one cause — nothing in the thirteen tables carries identity independent of what a source called something on the day we read it — so the model becomes an addressable-object registry with attributed, revisioned assertions, built in a **new** database (`devils_dictionary_v2`) with `devils_dictionary_dev` left intact as the matched baseline. Three things the gate settled by measuring rather than arguing. **The current revision is a partial `is_current` index, not a pointer** — 0.256 ms p95 against the pointer's 2.576 ms on the highest-degree node, because the partial index touches 41 buffers where the pointer touches 7,957; and *`DISTINCT ON` beat the pointer too*, which is the opposite of what the audit predicted, so neither claim gets repeated. **Lexical relations stay in the assertion model** — 1.16 M relations became 1.16 M assertions and 2.03 M revisions in 1,118 MB against 730 MB, 1.53× for a full history, so the typed-fallback table planned as an escape hatch is not needed. **A sense is identified by its content, never its position** — proved on the real Wiktionary `bank/noun/1`: delete a meaning from the middle and *"Money; profit."* moves from position 5 to 4 and keeps its identity, while position 5 resolves to a different sense and the curator's attachment still means what it meant. | #74, [ADR 0001](docs/adr/0001-encyclopedia-model.md) |
| 2026-09-08 | **Two things the gate found that no amount of reading would have.** The first draft of the registry guarded "exactly one subtype" with an `AFTER INSERT` trigger on `objects`; deleting Bierce's `entities` row left object 101 typed `entity` with no entity row *while `authored_by` still pointed at it* — so subtypes get `AFTER DELETE` guards too, and the rule that survives is that an identity is **retired**, never deleted out from under its claims. And declaring only `lexeme → lexeme` and `sense → sense` endpoints silently dropped **36,434 real relations** that are a meaning pointing at a *word* without naming which meaning — exactly what #74 §C says to preserve without inventing a sense id. Both are the argument for running an executable gate instead of reviewing a diagram. | #74 |
...
- **The UI history** — [#71](https://github.com/razrfly/dictionary/issues/71): sessions U0–U3 with their wireframes, the data contract for the word page (§7), and each session written up in full (§8a–§8d).
- **The mobile attestation** — [`docs/mobile/`](docs/mobile/README.md): scorecard row **U4**, twelve pages at 375 px with the screenshots that back it.
...
- **The community layer, sketched** — [`docs/sketches/`](docs/sketches/README.md): the `users` + `examples` + `votes` migration that was proven to fit and deliberately not shipped. Retired at #74 P1: the audit showed it cannot express a concept-to-concept example, and the real thing is now shipped schema.
...
- **The input manifest** — [`priv/sources/MANIFEST.json`](priv/sources/MANIFEST.json): every archived input pinned by SHA-256, verified by `mix dd.manifest`. The Kaikki dump is served from a rolling URL and cannot be re-fetched byte-identical, so its digest is the only thing that identifies it.
