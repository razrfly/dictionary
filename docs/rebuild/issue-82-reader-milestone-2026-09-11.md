...
- Entity pages fetch biography, works, definitions, editions, contents, and other connections independently. Each section has its own stable cursor; definitions render a compact first line; role edges are excluded from the raw connection wall.
...
- Bierce's first and second definition pages each contained 24 distinct rows with no overlap. The work section remained visible after paging definitions, and the query carried only `definitions_after`.
...
Closing #82 means this reader milestone is delivered. It does not authorize calling the original layered/participatory MVP complete; the owner-described Release C, D, and representative-entity follow-through remain separate work.
