...
makes the requests, the note that says what its results are, and an
`attribution_mark` for a licence that requires a mark shown, as GIPHY's terms
do — the same map a server provider returns from `attribution_mark/0`, read by
the same `Culture.mark/1` and drawn by the same component.
