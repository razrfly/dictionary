...
| findings | only claims a reader could see: current, active and through `Claims.visible(:public)`, the filter the card and the person page use, so a rejected or hidden credit is no agreement (#180 C1) |
| verdict | `Quotations.Badge.compute/2`: the badge by agreements, the #65 score by its signal table |
| review | `Claims.review/3` → `Verifier.reviewed/1`, in the review's transaction: a decision on a quotation's `authored_by` or `misattributed_to` recomputes the item's stored badge at once from the claims still public and the checks recorded on each remaining credit (no request), and moves the person's `refresh_after` to now so the next cron tick re-verifies them (#180 finding 1). An item with no stored badge is left without one |
